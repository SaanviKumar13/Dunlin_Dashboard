    ...
    <>
      <div className="p-0 md:p-4 mt-5">
        <div className=' items-center '>
          <div className='text-black-10 text-2xl font-medium font-figtree leading-8 mb-5'>Financial Transections</div>
          <div className="flex gap-4 py-4">
            ...
            <button class="bg-[#0B7B69] hover:bg-blue-700 text-white-10 font-bold py-4 border border-[#F6FFFA] px-6 rounded-md">
             <span className="text-md mr-3"> + </span> Import New Quickbooks Transactions
            </button>
          </div>
        </div>
        ...
        <div className="bg-white-30 p-4 rounded-lg">
            <div class="flex gap-4 py-4">
              <input type="text" id="large-input" class="block w-8/12 p-4 text-gray-900 rounded-lg bg-gray-50 text-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder='Type to search.....' />
          ...
          <div className="overflow-x-auto">
            <div className="flex rounded-md text-base font-figtree mb-2 p-3 bg-white-10">
              {columns.map((column, index) => (
                <div key={index} className={`flex-none text-gray-lightbulma figtree-font text-lg font-semibold leading-6 p-2 px-3 ${index === 0 ? 'pe-12' : ''} ${index === 0 ? '' : 'w-3/5 md:w-1/4 '}`}>
                  {column}
                </div>
              ))}
            ...
            {data.map((row, index) => (
              <div key={index} className="flex max-w-screen rounded-md text-base font-figtree mb-2 p-2 w-full">
